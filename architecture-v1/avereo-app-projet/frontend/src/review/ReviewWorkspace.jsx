...
export default function ReviewWorkspace() {
  ...
  const trackingOnly = data.trackingOnly === true;
  ...
  return <main className="review-workspace">
    ...
    <div className={`review-layout${trackingOnly ? ' tracking-only' : ''}`}>
      ...
      <section id="phase-review" tabIndex={-1} className="phase-content" aria-label={`Revue de la phase ${selected}`}>
        ...
        {!trackingOnly && stale && <p className="feedback error">Un livrable a changé après sa validation. L’accord de passage est bloqué ; demander une nouvelle revue.</p>}
        {!trackingOnly && <><div className="section-tabs" role="tablist" aria-label="Contenu de la revue">{[['livrables', 'Livrables'], ['criteres', `Critères · ${phase.exitCriteria.length}`], ['journal', `Journal · ${events.length}`]].map(([id, label]) => <button key={id} id={`tab-${id}`} role="tab" aria-selected={section === id} aria-controls={`panel-${id}`} onClick={() => setSection(id)}>{label}</button>)}</div>
