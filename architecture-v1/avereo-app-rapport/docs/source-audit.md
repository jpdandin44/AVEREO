...
| AMELIORATION | `frontend/package.json` | La source ne definissait aucun test unitaire frontend. | Une premiere suite `node:test` couvre la sous-categorie, sa normalisation et la migration du prototype Habitologie; le lint et les tests de composants restent a definir dans la strategie globale. | mitige |
