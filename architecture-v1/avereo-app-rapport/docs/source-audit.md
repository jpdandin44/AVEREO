...
| AMELIORATION | `frontend/package.json` | La source ne definissait aucun test unitaire frontend. | Une premiere suite `node:test` couvre le type, la version et la normalisation du Rapport d'habitologie; le lint et les tests de composants restent a definir dans la strategie globale. | mitige |
