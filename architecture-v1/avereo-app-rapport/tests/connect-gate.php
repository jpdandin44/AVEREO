...
$directory = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'avereo-rapport-gate-' . bin2hex(random_bytes(8));
$secret = str_repeat('rapport-gate-secret-', 2);
$config = [
    'environment' => 'local',
    'connect_launch_secret' => $secret,
    'connect_launch_nonce_directory' => $directory,
    'connect_launch_max_seconds' => 300,
    'connect_gate_cookie' => 'AVEREO_RAPPORT_GATE_TEST',
...
gate_test_assert(
    $standardUser['roles'] === ['utilisateur_rapport'],
...
gate_test_assert(
    $adminUser['roles'] === ['administrateur_rapport'],
...
$wrongAppTicket = gate_test_ticket('coupe', $secret, avereo_gate_base64url_encode(random_bytes(24)));
try {
    $payload = avereo_gate_decode_signed($wrongAppTicket, $secret);
    avereo_gate_assert_payload($payload, 300);
    throw new RuntimeException('Un ticket Coupe ne doit pas ouvrir Rapport.');
} catch (RuntimeException $exception) {
    gate_test_assert(
...
fwrite(STDOUT, "PASS rapport CONNECT gate\n");
